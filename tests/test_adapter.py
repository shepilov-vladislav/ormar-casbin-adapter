...
# Thirdparty:
from casbin import Adapter, Enforcer, Model
from databases import Database

# Firstparty:
from ormar_casbin_adapter import DatabasesAdapter
from ormar_casbin_adapter.adapter import Filter
...
async def test_is_filtered(
    db: Database, setup_policies, model_conf_path, casbin_rule_table
):
    adapter1 = DatabasesAdapter(model=casbin_rule_table)
    enforcer1 = Enforcer(model_conf_path, adapter1)
    await enforcer1.load_policy()
    assert not enforcer1.is_filtered()

    adapter1 = DatabasesAdapter(model=casbin_rule_table, filtered=True)
    enforcer1 = Enforcer(model_conf_path, adapter1)
    await enforcer1.load_policy()
